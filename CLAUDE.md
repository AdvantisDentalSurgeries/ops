...
| Auth | Supabase Auth → JWT validated in Express middleware |
| Email | Nodemailer (SMTP) or Resend |

**Three user roles**: `OFFICE_MANAGER`, `DENTIST`, `PATIENT` — each gets its own frontend portal with separate route guards enforced via `AuthContext` (Supabase session + role claim).

**Request lifecycle**:
1. User authenticates via Supabase Auth on the frontend → receives JWT
2. Frontend sends JWT as `Authorization: Bearer <token>` on all API requests
3. Express middleware validates JWT, extracts role, attaches to `req.user`
4. Route handler delegates to a service function
5. Service enforces business rules, then calls Prisma for DB operations
...
The Prisma schema lives at `backend/prisma/schema.prisma` (planned). See `analysis.md` (FR1–FR16) for the full functional requirements and `architecture/diagram.md` for Mermaid ER and sequence diagrams.
...
All routes prefixed `/api/`. Auth endpoints hit Supabase directly from the frontend.
...
The backend will need a `.env` (not committed) with Supabase URL/keys, JWT secret, and SMTP credentials. An `.env.example` should be created alongside any `.env` usage.
